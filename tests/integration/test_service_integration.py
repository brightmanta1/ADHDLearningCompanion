...
class TestServiceIntegration:
    ...
    async def test_full_workflow(self, service_manager):
        ...
        text_request = {
            ...
            "content": {"text": "Test content for processing with ADHD considerations"},
        }
    ...
    async def test_celery_integration(self, service_manager):
        ...
        task = celery_app.send_task(
            "process_content", args=["text_processing", {"text": "Test content"}]
        )
    ...
    async def test_redis_caching(self, service_manager):
        ...
        # Сохранение в кэш
        redis_client.set(cache_key, str(cache_data), ex=300)  # 5 минут
    ...
    async def test_error_handling(self, service_manager):
        ...
        # Некорректный запрос
        invalid_request = {"type": "invalid_type", "content": {}}
        ...
        assert (
            MetricsCollector.REQUEST_COUNT.labels(
                type="invalid_type", status="error"
            )._value.get()
            > 0
        )
