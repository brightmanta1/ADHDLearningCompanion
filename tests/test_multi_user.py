...
async def test_resource_allocation(resource_manager):
    ...
    task = Task(
        ...
        resource_requirements={ResourceType.CPU: 20.0, ResourceType.MEMORY: 512.0},
...
async def test_service_request_handling(service_manager):
    ...
    request_data = {
        ...
        "priority": "medium",
        "content": {"text": "Test text", "language": "ru"},
    }
...
async def test_concurrent_requests(service_manager):
    ...
    async def make_request(user_id: str):
        request_data = {
            "type": "text_processing",
            "priority": "medium",
            "content": {"text": f"Test text for {user_id}", "language": "ru"},
        }
...
async def test_task_priority(resource_manager):
    ...
    for priority in TaskPriority:
        task = Task(
            ...
            session_id="test_session",
            priority=priority,
            resource_requirements={ResourceType.CPU: 10.0, ResourceType.MEMORY: 256.0},
            created_at=datetime.now(),
            timeout=30.0,
            coroutine=asyncio.sleep(0.1),
        )
