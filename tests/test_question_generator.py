...
async def test_generate_questions(question_generator):
    ...
    with patch.object(
        question_generator, "_generate_multiple_choice"
    ) as mock_mc, patch.object(
        question_generator, "_generate_true_false"
    ) as mock_tf, patch.object(
        question_generator, "_generate_open_ended"
    ) as mock_oe, patch.object(
        question_generator, "_add_explanation"
    ) as mock_explain:
...
def test_generate_multiple_choice(question_generator):
    ...
    with patch.object(
        question_generator.model, "generate"
    ) as mock_generate, patch.object(
        question_generator.tokenizer, "decode"
    ) as mock_decode:
        ...
        with patch.object(
            question_generator, "_generate_answer_options"
        ) as mock_options:
...
def test_calculate_difficulty(question_generator):
    ...
    assert (
        question_generator._calculate_difficulty(
            "", "What are the primary characteristics of Python?"
        )
        == "medium"
    )
    ...
    assert (
        question_generator._calculate_difficulty(
            "", "What are the implications of using asynchronous programming paradigms?"
        )
        == "hard"
    )
...
def test_negate_statement(question_generator):
    ...
    # Простое отрицание
    assert "is not" in question_generator._negate_statement(
        "Python is a programming language"
    )
    ...
    assert question_generator._negate_statement(statement).startswith(
        "It is not true that"
    )
