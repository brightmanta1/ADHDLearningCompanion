...
async def test_handle_request(service_manager):
    ...
    service_manager.database.create_user_profile = Mock(return_value={
        "user_id": user_id,
        "preferences": {}
    })
...
def test_determine_priority(service_manager):
    ...
    assert service_manager._determine_priority({"priority": "high"}) == TaskPriority.HIGH
...
def test_calculate_resource_requirements(service_manager):
    ...
    # Тест для видео
    video_req = service_manager._calculate_resource_requirements({"type": "video_processing"})
    assert video_req[ResourceType.CPU] == 30.0
    assert video_req[ResourceType.GPU] == 50.0
    assert video_req[ResourceType.MEMORY] == 1024.0
    
    # Тест для текста
    text_req = service_manager._calculate_resource_requirements({"type": "text_processing"})
