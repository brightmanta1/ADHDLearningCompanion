...
async def test_process_video_success(video_processor, sample_video_url):
    ...
    with patch("yt_dlp.YoutubeDL") as mock_ydl, patch.object(
        video_processor, "_extract_audio"
    ) as mock_extract_audio, patch.object(
        video_processor, "_transcribe_audio"
    ) as mock_transcribe_audio, patch.object(
        video_processor, "_cleanup_temp_files"
    ) as mock_cleanup:
...
def test_segment_video(video_processor, mock_whisper_result):
    ...
    with patch.object(
        video_processor, "_extract_key_points"
    ) as mock_extract_points, patch.object(
        video_processor, "_create_thumbnail"
